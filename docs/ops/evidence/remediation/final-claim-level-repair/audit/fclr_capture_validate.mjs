#!/usr/bin/env node
/**
 * Final Claim-Level Repair — full public-article validation + claim integrity.
 *
 * Env:
 *   FCLR_PREVIEW (required)
 *   FCLR_COMMIT (application SHA)
 *   FCLR_DEPLOYMENT (dpl_…)
 */
...
const PREVIEW = process.env.FCLR_PREVIEW || process.env.IPI_PREVIEW;
const COMMIT = process.env.FCLR_COMMIT || process.env.IPI_COMMIT || "";
const DEPLOYMENT_ID = process.env.FCLR_DEPLOYMENT || process.env.IPI_DEPLOYMENT || "";
if (!PREVIEW || !COMMIT || !DEPLOYMENT_ID) {
  console.error("FCLR_PREVIEW, FCLR_COMMIT, FCLR_DEPLOYMENT required");
  process.exit(1);
}
...
const MEASURED_CLAIM_RE = [
  /\bMost licensing failures\b/i,
  /\bfirst six months of operation\b/i,
  /\bwhere most failures occur\b/i,
  /\bMost companies have compliance on paper\b/i,
  /\bFew have compliance that actually functions\b/i,
  /\bprovider exits are common\b/i,
  /\bInternational experience consistently demonstrates\b/i,
  /\bapproximately USD 115 billion\b/i,
  /\badditional USD 1\.14 billion\b/i,
  /\braises BRL 1\.14 billion\b/i,
  /\bMost licensing failures are not rejections\b/i,
];

const B2B_SLUG = "brazil-b2b-registration-requirement-betting-igaming";
const B2B_EXPECTED_TITLE =
  "Brazil B2B Provider Registration: Consultation Status and What Remains Unconfirmed";

function scanMeasuredClaims(body) {
  return MEASURED_CLAIM_RE.filter((re) => re.test(body)).map((re) => re.toString());
}
...
function scanB2BClaimMapping(body, h1, title) {
  const failures = [];
  if (!/Aviso de Consulta Pública SPA\/MF nº 1\/2026|Aviso SPA\/MF nº 1\/2026/i.test(body)) {
    failures.push("missing_dou_aviso_reference");
  }
  if (!/consultation[\s\S]{0,40}completed|completed[\s\S]{0,40}consultation/i.test(body)) {
    failures.push("missing_consultation_completed");
  }
  if (!/final binding ordinance is not confirmed|not confirmed as published/i.test(body)) {
    failures.push("missing_final_ordinance_unconfirmed");
  }
  if (/ICE Barcelona/i.test(body)) {
    failures.push("ice_barcelona_event_retained");
  }
  if (/Increasingly, this type of registration is necessary/i.test(body)) {
    failures.push("dudena_quote_retained");
  }
  if (h1 !== B2B_EXPECTED_TITLE) {
    failures.push(`h1_title_mismatch:${h1}`);
  }
  if (!String(title || "").startsWith(B2B_EXPECTED_TITLE)) {
    failures.push(`document_title_mismatch:${title}`);
  }
  return failures;
}

function scanUnsupportedEventAttributions(body, slug) {
  const failures = [];
  if (slug === B2B_SLUG) {
    if (/ICE Barcelona/i.test(body)) failures.push("ice_barcelona");
    if (/Regis Dudena[\s\S]{0,80}signalled|Dudena[\s\S]{0,40}stated/i.test(body)) {
      failures.push("dudena_oral_attribution");
    }
  }
  return failures;
}
...
async function main() {
  ...
  const results = {
    phase: "final-claim-level-repair",
    preview: PREVIEW,
  ...
  const editorial_measured_claim_failures = [];
  const unsourced_direct_quotes = [];
  const source_claim_mapping_failures = [];
  const unsupported_event_attributions = [];
  let editorial_routes_tested = 0;
  let duplicate_og = [];
  ...
  for (const slug of PUBLIC_SLUGS) {
    ...
    if (isEditorial) {
      editorial_routes_tested += 1;
      if (!evaled.hasEditorialBasis) {
        public_routes_without_sources_or_basis.push({ slug, kind: "editorial_missing_basis" });
      }
      const measured = scanMeasuredClaims(evaled.body);
      if (measured.length) {
        editorial_measured_claim_failures.push({ slug, patterns: measured });
      }
      const quotes = scanUnsourcedQuotes(evaled.body, slug, true);
      if (quotes.length) {
        unsourced_direct_quotes.push({ slug, reasons: quotes });
      }
    } else {
    ...
    if (slug === B2B_SLUG) {
      const mapFails = scanB2BClaimMapping(evaled.body, evaled.h1, evaled.statusTitle);
      if (mapFails.length) {
        source_claim_mapping_failures.push({ slug, reasons: mapFails });
      }
      const eventFails = scanUnsupportedEventAttributions(evaled.body, slug);
      if (eventFails.length) {
        unsupported_event_attributions.push({ slug, reasons: eventFails });
      }
      const quotes = scanUnsourcedQuotes(evaled.body, slug, false);
      if (quotes.length) {
        unsourced_direct_quotes.push({ slug, reasons: quotes });
      }
    }
  ...
  const zeroBags = {
    ...
    editorial_measured_claim_failures: editorial_measured_claim_failures.length,
    unsourced_direct_quotes: unsourced_direct_quotes.length,
    source_claim_mapping_failures: source_claim_mapping_failures.length,
    unsupported_event_attributions: unsupported_event_attributions.length,
  };
  ...
  const validation = {
    ...
    editorial_measured_claim_failures_detail: editorial_measured_claim_failures,
    unsourced_direct_quotes_detail: unsourced_direct_quotes,
    source_claim_mapping_failures_detail: source_claim_mapping_failures,
    unsupported_event_attributions_detail: unsupported_event_attributions,
  ...
  fs.writeFileSync(path.join(AUDIT, "FCLR_CAPTURE.json"), JSON.stringify(results, null, 2));
  fs.writeFileSync(path.join(AUDIT, "FCLR_VALIDATION.json"), JSON.stringify(validation, null, 2));
