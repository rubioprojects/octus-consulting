#!/usr/bin/env node
/**
 * Phase 5C Intelligence / Insights / Legal — capture + validation against immutable preview.
 *
 * Env:
 *   PHASE5C_PREVIEW (required)
 *   PHASE5C_COMMIT (application SHA)
 *   PHASE5C_DEPLOYMENT (dpl_…)
 *   PHASE5C_DEPLOYED_SHA (optional deployment git sha)
 */
...
const PREVIEW = process.env.PHASE5C_PREVIEW;
const COMMIT = process.env.PHASE5C_COMMIT || "";
const DEPLOYMENT_ID = process.env.PHASE5C_DEPLOYMENT || "";
const DEPLOYED_SHA = process.env.PHASE5C_DEPLOYED_SHA || COMMIT;
if (!PREVIEW || !COMMIT || !DEPLOYMENT_ID) {
  console.error("PHASE5C_PREVIEW, PHASE5C_COMMIT, PHASE5C_DEPLOYMENT required");
  process.exit(1);
}
...
/** Full chapter capture: hubs + legal + not-found */
const ROUTES = [
  { route: "/intelligence", template: "intelligence-hub" },
  { route: "/insights", template: "insights-index" },
  { route: "/privacy", template: "legal-policy" },
  { route: "/terms", template: "legal-policy" },
  { route: "/cookies", template: "legal-policy" },
  { route: "/compliance-channel", template: "compliance-channel" },
  { route: "/this-page-does-not-exist-phase5c", template: "not-found", expectStatus: 404 },
];

/** All 33 insight slugs from lib/posts.ts (public inventory SoT). */
const ARTICLE_SLUGS = [
...
/** Full chapter set across baseline/before/reconciled (runtime budget). */
...
/** Reconciled-only article screenshots (non-exemplar) — keep runtime feasible. */
const ARTICLE_DESKTOP_CHAPTERS = [
  { id: "full", fullPage: true },
  { id: "hero", selector: ".octus-dark-hero, .page-hero, main > section:first-of-type", height: 900 },
];

const ARTICLE_MOBILE_CHAPTERS = [
  { id: "full", fullPage: true },
  { id: "first-fold", selector: "main > section:first-of-type", height: 844 },
];
...
const BEFORE = "https://octus-consulting-jhedve7t0-axle1.vercel.app";
const BEFORE_COMMIT = "eb37d6f96db6bda418e4f8132cbcda6b6731deaf";
const BEFORE_DPL = "dpl_HpNbyjdhxuLLrrcpitB4DA1EJ2TX";
/** Phase 5C starting HEAD (accepted Phase 5B evidence tip) — frozen-diff baseline */
const PHASE5C_START = "cee3e907069542b14c85bec56769dc9549b4674c";
...
/** Must not change since PHASE5C_START. */
const FROZEN_PATHS = [
  "app/page.tsx",
  ...
  "components/system/CtaButton.tsx",
...
/**
 * Allowed Phase 5C mutation surface (not frozen).
 * lib/commercial.ts: INTELLIGENCE-only — verified separately if present in diff.
 */
const ALLOWED_PATHS = [
  "app/intelligence/",
  "app/insights/",
  ...
  "app/terms/",
  "app/cookies/",
  "app/compliance-channel/",
  "app/not-found.tsx",
  "lib/posts.ts",
  "lib/commercial.ts",
  "lib/insightEnrichment.ts",
  "lib/intelligence.ts",
  "docs/ops/evidence/remediation/intelligence-legal/",
...
async function runAxe(page, route, vpName, a11yResults) {
  try {
...
async function captureRouteSet({
  ...
  for (const [cls, base, commit, dpl] of classifications) {
    for (const [vpName, vp] of Object.entries(VIEWPORTS)) {
      ...
      if (cls === "reconciled" && runReconciledAudits) {
        ...
        await runAxe(page, route, vpName, a11yResults);
        ...
        if (vpName === "desktop") {
          const m = await page.evaluate(() => {
            ...
            const wa = !!document.querySelector(".wa-float, a.wa-float");
...
async function main() {
  ...
  const classificationsReconciledOnly = [
    ["reconciled", PREVIEW, COMMIT, DEPLOYMENT_ID],
  ];

  console.log("Capturing chapter routes (baseline + before + reconciled)...");
  for (const routeSpec of ROUTES) {
    await captureRouteSet({
      ...
      prohibitedHits,
      overflow,
      runReconciledAudits: true,
    });
  }

  console.log("Capturing exemplar articles (full chapters × baseline/before/reconciled)...");
  for (const routeSpec of ARTICLE_ROUTES.filter((r) => EXEMPLAR_SLUGS.includes(r.slug))) {
    await captureRouteSet({
      browser,
      routeSpec,
      classifications: classificationsFull,
  ...
  console.log("Capturing remaining articles (reconciled only, limited chapters)...");
  for (const routeSpec of ARTICLE_ROUTES.filter((r) => !EXEMPLAR_SLUGS.includes(r.slug))) {
    await captureRouteSet({
      browser,
      routeSpec,
      classifications: classificationsReconciledOnly,
      chaptersFor: (vpName) =>
        vpName === "mobile" ? ARTICLE_MOBILE_CHAPTERS : ARTICLE_DESKTOP_CHAPTERS,
      manifestItems,
      a11yResults,
      crawl,
      meta,
  ...
  const homeEval = await homePage.evaluate(() => {
    ...
    const cookie = !!document.querySelector(".cookie-banner, [class*='cookie']");
    const wa = !!document.querySelector(".wa-float, a.wa-float");
    return { text, moduleButtons, emDashes, ctas, header, footer, cookie, wa };
  });
  await homeCtx.close();
  ...
  const homeResidual = homeEval.emDashes;
  ...
  const frozenDiff = execSync(`git diff --name-only ${PHASE5C_START}..${COMMIT}`, {
    cwd: REPO,
    encoding: "utf8",
  })
    .trim()
  ...
  const frozenHits = frozenDiff.filter((f) =>
    FROZEN_PATHS.some((p) => pathMatches(f, p))
  );

  let commercialIntelligenceOnly = true;
  let commercialDiffNote = "unchanged";
  if (frozenDiff.includes("lib/commercial.ts")) {
    const patch = execSync(`git diff ${PHASE5C_START}..${COMMIT} -- lib/commercial.ts`, {
      cwd: REPO,
      encoding: "utf8",
    });
    commercialDiffNote = "changed";
    // Fail-closed: commercial.ts may change only when the patch is intelligence-scoped.
    const exportTouches = [...patch.matchAll(/^[-+]export const ([A-Za-z0-9_]+)/gm)].map(
      (m) => m[1]
    );
    const nonIntelExports = exportTouches.filter((name) => !/intelligence/i.test(name));
    const touchesIntelligence = /INTELLIGENCE|intelligence/i.test(patch);
    commercialIntelligenceOnly = touchesIntelligence && nonIntelExports.length === 0;
  }
  ...
  const crawlOk = crawlEntries.every(([route, s]) => {
    const spec = [...ROUTES, ...ARTICLE_ROUTES].find((r) => r.route === route);
  ...
  const regression = {
    homepage: {
      ...
      home_punctuation_exactly_one: homeResidual === 1 ? "PASS" : "FAIL",
      home_residual_count: homeResidual,
    },
    team: {
      team_exact_10: teamExact10 ? "PASS" : "FAIL",
      band_labels: bandLabels,
    },
    services: {
      solutions_200: servicesOk.every((d) => d.status === 200 || d.status === 308 || d.status === 301)
        ? "PASS"
        : "FAIL",
      routes: servicesOk,
    },
    phase5a: {
      routes_200: phase5aOk.every((d) => d.status === 200 || d.status === 308 || d.status === 301)
        ? "PASS"
        : "FAIL",
      routes: phase5aOk,
    },
    phase5b: {
      routes_200: phase5bOk.every((d) => d.status === 200 || d.status === 308 || d.status === 301)
        ? "PASS"
        : "FAIL",
      routes: phase5bOk,
    },
  ...
  const regressionFlatPass =
    ...
    regression.homepage.home_punctuation_exactly_one === "PASS" &&
    regression.team.team_exact_10 === "PASS" &&
  ...
  const checks = {
    ...
    commercial_intelligence_only: commercialIntelligenceOnly ? "PASS" : "FAIL",
    home_residual_exactly_one: homeResidual === 1 ? "PASS" : "FAIL",
    home_regression_surface: homeSevenSignals ? "PASS" : "FAIL",
    banking_not_peer: bankingNotPeer ? "PASS" : "FAIL",
    ...
    duplicate_commercial_labels: ctaRows.every((r) => {
      const commercial = r.links.filter(
        (l) =>
          /→$/.test(l.text) &&
  ...
  const summary = {
    ...
    article_routes: ARTICLE_ROUTES.map((r) => r.route),
    ...
    bound_residuals: {
      HOME_PUNCTUATION_RESIDUAL_001: {
        count: homeResidual,
        status: homeResidual === 1 ? "OPEN_EXACTLY_ONE" : "ANOMALY",
      },
      GLOBAL_A11Y_REGION_RESIDUAL_001: {
        status: "OPEN_BOUND",
        note: "Axe moderate region on .wa-float recorded; not treated as Phase 5C failure",
      },
    ...
    allowed_paths_note: ALLOWED_PATHS,
    commercial_diff: commercialDiffNote,
    ...
    JSON.stringify(
      {
        ...
        immutable_preview_url: PREVIEW,
        deployment_id: DEPLOYMENT_ID,
        phase5c_application_commit: COMMIT,
        screenshot_count: manifestItems.length,
        items: manifestItems,
      },
      null,
  ...
  fs.writeFileSync(path.join(AUDIT, "PHASE5C_VALIDATION.json"), JSON.stringify(summary, null, 2) + "\n");
  fs.writeFileSync(
    path.join(AUDIT, "PHASE5C_A11Y.json"),
    JSON.stringify(
      { generated_at: summary.generated_at, axe_core_version: axeCoreVersion, results: a11yResults },
      null,
      2
    ) + "\n"
  );
  fs.writeFileSync(
    path.join(AUDIT, "PHASE5C_CRAWL.json"),
    JSON.stringify({ generated_at: summary.generated_at, crawl, status: checks.crawl_status }, null, 2) +
      "\n"
  );
  fs.writeFileSync(
    path.join(AUDIT, "PHASE5C_METADATA.json"),
    JSON.stringify({ generated_at: summary.generated_at, routes: meta }, null, 2) + "\n"
  );
  fs.writeFileSync(
    path.join(AUDIT, "PHASE5C_OVERFLOW.json"),
    JSON.stringify({ generated_at: summary.generated_at, overflow }, null, 2) + "\n"
  );
  fs.writeFileSync(
    path.join(AUDIT, "PHASE5C_PROHIBITED.json"),
    JSON.stringify(
      {
        generated_at: summary.generated_at,
  ...
  fs.writeFileSync(
    path.join(AUDIT, "PHASE5C_REGRESSION.json"),
    JSON.stringify(
      {
        generated_at: summary.generated_at,
        regression,
