#!/usr/bin/env node
/**
 * Phase 5B Markets/Jurisdictions/Brazil — capture + validation against immutable preview.
 *
 * Env:
 *   PHASE5B_PREVIEW (required)
 *   PHASE5B_COMMIT (application SHA)
 *   PHASE5B_DEPLOYMENT (dpl_…)
 *   PHASE5B_DEPLOYED_SHA (optional deployment git sha)
 */
...
const PREVIEW = process.env.PHASE5B_PREVIEW;
const COMMIT = process.env.PHASE5B_COMMIT || "";
const DEPLOYMENT_ID = process.env.PHASE5B_DEPLOYMENT || "";
const DEPLOYED_SHA = process.env.PHASE5B_DEPLOYED_SHA || COMMIT;
if (!PREVIEW || !COMMIT || !DEPLOYMENT_ID) {
  console.error("PHASE5B_PREVIEW, PHASE5B_COMMIT, PHASE5B_DEPLOYMENT required");
  process.exit(1);
}
...
const ROUTES = [
  { route: "/markets", template: "markets-overview" },
  ...
  { route: "/jurisdictions", template: "jurisdictions-overview" },
  ...
  { route: "/brazil", template: "brazil-strategic-market" },
];
...
const BEFORE = "https://octus-consulting-nx5t8xtoo-axle1.vercel.app";
const BEFORE_COMMIT = "9e97c13e90ecc4165e969ee04181e8d2a3a9110c";
const BEFORE_DPL = "dpl_C8Zt5mFCF6BaK165cKMt6ELBknKj";
/** Phase 5A starting HEAD (accepted Phase 4 evidence tip) — frozen-diff baseline */
const PHASE5B_START = "9b5ce63501e48e4da0440ae5f3f59f776d2bf955";
...
async function main() {
  ...
  const frozenDiff = execSync(`git diff --name-only ${PHASE5B_START}..${COMMIT}`, {
    cwd: REPO,
    encoding: "utf8",
  })
    .trim()
  ...
  const incomplete = a11yResults.reduce((n, r) => n + (r.incomplete || 0), 0);
    ...
    try {
      const hdr = await httpMeta(originBase(PREVIEW) + "/");
      previewNoindex = /noindex/i.test(hdr.xRobots || "") ? "PASS" : "FAIL";
    } catch {
      previewNoindex = "FAIL";
    }
  ...
  const summary = {
    ...
    bound_residuals: {
      ...
      GLOBAL_A11Y_REGION_RESIDUAL_001: {
        status: "OPEN_BOUND",
        note: "Axe moderate region on .wa-float recorded; not treated as Phase 5B failure",
      },
  ...
  fs.writeFileSync(path.join(ROOT, "SCREENSHOT_MANIFEST.json"), JSON.stringify({
    ...
    immutable_preview_url: PREVIEW,
    deployment_id: DEPLOYMENT_ID,
    phase5b_application_commit: COMMIT,
    screenshot_count: manifestItems.length,
    items: manifestItems,
  }, null, 2) + "\n");

  fs.writeFileSync(path.join(AUDIT, "PHASE5B_VALIDATION.json"), JSON.stringify(summary, null, 2) + "\n");
  fs.writeFileSync(path.join(AUDIT, "PHASE5B_A11Y_AUDIT.json"), JSON.stringify({ generated_at: summary.generated_at, axe_core_version: axeCoreVersion, results: a11yResults }, null, 2) + "\n");
  fs.writeFileSync(path.join(AUDIT, "PHASE5B_CTA_AUDIT.json"), JSON.stringify({ generated_at: summary.generated_at, routes: ctaRows }, null, 2) + "\n");
  fs.writeFileSync(path.join(AUDIT, "PHASE5B_METADATA_AUDIT.json"), JSON.stringify({ generated_at: summary.generated_at, routes: meta }, null, 2) + "\n");
  fs.writeFileSync(path.join(AUDIT, "PHASE5B_OVERFLOW_AUDIT.json"), JSON.stringify({ generated_at: summary.generated_at, overflow }, null, 2) + "\n");
